...
const SeekerDashboard: React.FC = () => {
  ...
  const priceCalculation = useMemo(() => {
    ...
    if (hours > 0 && hours * pricing.hourly > pricing.daily) {
      hours = 0;
      days += 1;
    }
    if (days >= 30) {
    ...
    if (remainderCost > pricing.monthly) {
      days = 0;
      hours = 0;
      months += 1;
    }
    ...
    const total =
      months * pricing.monthly + days * pricing.daily + hours * pricing.hourly;
    let parts: string[] = [];
  ...
  const handleBook = async (space: ParkingSpace) => {
    ...
    const nid = notify("Processing reservation...", "info", [
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (!spaceDoc.exists())
          throw new Error("Space listing no longer exists.");
        ...
        if (vehicleType === "car" && currentCarSlots <= 0)
          throw new Error("Car slots are now full.");
        if (vehicleType === "bike" && currentBikeSlots <= 0)
          throw new Error("Bike slots are now full.");

        const updatedCarSlots =
          vehicleType === "car" ? currentCarSlots - 1 : currentCarSlots;
        const updatedBikeSlots =
          vehicleType === "bike" ? currentBikeSlots - 1 : currentBikeSlots;
        ...
        updateNotify(nid, {
          currentStep: 2,
          message: "Updating listing availability...",
        });
    ...
    <div className="py-12 w-full animate-fade-in-up">
      <div className="mb-12">
        ...
        <p className="text-slate-500 text-2xl font-medium">
          Rent convenient parking near your destination in real-time.
        </p>
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-12">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-80 bg-white border border-slate-100 rounded-[3rem] animate-pulse"
            ></div>
          ))}
        </div>
        ...
        <div className="text-center py-40 bg-white rounded-[4rem] border border-slate-100 shadow-sm">
          ...
          <p className="text-slate-400 font-black text-3xl tracking-tight">
            No parking spots found in this sector.
          </p>
          <button
            onClick={() => setSearchTerm("")}
            className="mt-8 text-yellow-600 font-black uppercase tracking-widest text-sm hover:text-yellow-700 transition-colors"
          >
            ...
            >
              <div className="h-56 xl:h-64 bg-slate-200 relative overflow-hidden m-4 rounded-[2.2rem]">
                ...
                <div className="absolute top-6 right-6 flex flex-col gap-3 items-end">
                  <div
                    className={`px-6 py-2 rounded-full text-xs font-black border shadow-2xl flex items-center gap-3 ${space.carSlots > 0 ? "bg-white/95 text-slate-700" : "bg-red-500 text-white border-red-400"}`}
                  >
                    <Car
                      size={18}
                      className={
                        space.carSlots > 0 ? "text-yellow-500" : "text-white"
                      }
                    />{" "}
                    {space.carSlots || 0}
                  </div>
                  <div
                    className={`px-6 py-2 rounded-full text-xs font-black border shadow-2xl flex items-center gap-3 ${space.bikeSlots > 0 ? "bg-white/95 text-slate-700" : "bg-red-500 text-white border-red-400"}`}
                  >
                    <Bike
                      size={18}
                      className={
                        space.bikeSlots > 0 ? "text-blue-500" : "text-white"
                      }
                    />{" "}
                    {space.bikeSlots || 0}
                  </div>
              ...
              <div className="p-10 flex-1 flex flex-col">
                <h3 className="font-black text-3xl text-slate-900 truncate mb-2 leading-none">
                  {space.title}
                </h3>
                ...
                <div className="flex items-center justify-between mt-auto">
                  <div className="flex flex-wrap gap-2">
                    {space.amenities.map((a) => (
                      <AmenityTag key={a} name={a} />
                    ))}
                  </div>
                  <div className="text-yellow-600 font-black text-2xl ml-4 whitespace-nowrap">
      ...
      {selectedSpace && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div
            ...
            onClick={() => !bookingLoading && setSelectedSpace(null)}
          ></div>
            ...
            <div className="p-12 lg:p-16">
              <div className="flex justify-between items-center mb-10">
                <div>
                  ...
                  <p className="text-slate-400 font-medium text-lg truncate max-w-[280px]">
                    {selectedSpace.title}
                  </p>
                </div>
                <button
                  onClick={() => setSelectedSpace(null)}
                  className="p-4 bg-slate-50 hover:bg-slate-100 rounded-full transition-all text-slate-400"
                >
                  <CloseIcon size={28} />
                </button>
              </div>
              {!user ? (
                <div className="text-center py-10">
                  ...
                  <p className="text-slate-900 font-black mb-4 text-2xl">
                    Identity Required
                  </p>
                  <p className="text-slate-500 font-medium text-lg mb-10">
                    You need an account to book parking spots and contact hosts.
                  </p>
                  <button
                    onClick={() => window.location.reload()}
                    className="w-full py-6 bg-yellow-400 text-slate-900 font-black rounded-3xl shadow-2xl shadow-yellow-400/20 active:scale-95 transition-all text-xl"
                  >
                    SIGN IN TO BOOK
                  </button>
                </div>
                ...
                <div className="space-y-10">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full p-5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-black outline-none focus:border-yellow-400 transition-all"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-400 mb-3 uppercase tracking-widest">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <button
                      ...
                      disabled={selectedSpace.carSlots <= 0}
                      className={`flex items-center gap-4 p-8 rounded-[2.5rem] border-2 transition-all ${vehicleType === "car" ? "border-yellow-400 bg-yellow-50 text-slate-900 shadow-xl" : "border-slate-50 text-slate-400"}`}
                    >
                      <Car size={32} />
                      <span className="font-black text-lg">CAR</span>
                    </button>
                    <button
                      onClick={() => setVehicleType("bike")}
                      disabled={selectedSpace.bikeSlots <= 0}
                      className={`flex items-center gap-4 p-8 rounded-[2.5rem] border-2 transition-all ${vehicleType === "bike" ? "border-yellow-400 bg-yellow-50 text-slate-900 shadow-xl" : "border-slate-50 text-slate-400"}`}
                    >
                      <Bike size={32} />
                      <span className="font-black text-lg">BIKE</span>
                    </button>
                  </div>
                    ...
                    <div className="flex justify-between items-center mb-8">
                      <div>
                        ...
                        <span className="text-5xl font-black text-yellow-400">
                          ₹{priceCalculation.total.toFixed(0)}
                        </span>
                      </div>
                      <div className="text-right font-black text-slate-500 text-[11px] uppercase tracking-widest">
                        {priceCalculation.label}
                      </div>
                    </div>
                    <button
                      onClick={() => handleBook(selectedSpace)}
                      disabled={bookingLoading || priceCalculation.total <= 0}
                      className="w-full py-6 bg-yellow-400 hover:bg-yellow-500 text-slate-900 font-black rounded-[2rem] transition-all flex items-center justify-center gap-4 active:scale-95 disabled:opacity-50 text-xl tracking-widest uppercase"
                    >
                      {bookingLoading ? (
